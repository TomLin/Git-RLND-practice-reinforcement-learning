...
def collect_trajectories(model, env, brain_name, init_states, episode_end, n_steps):
    ...
    while True:
        ...
        batch_s.append(states) # final shape of batch_s (T, n_process, state_size; list of tensor)
        batch_a.append(actions) # final shape of batch_a (T, n_process, action_size; list of tensor)
        batch_r.append(rewards) # final shape of batch_r (T, n_process; list of numpy array)

        if dones.any() or t >= n_steps:
            model.eval()
            next_states = torch.from_numpy(next_states).float()
            final_r = model.get_state_value(next_states).detach().numpy() # final_r (n_process, ; numpy)
            model.train()
    ...
    if dones.any():
        ...
        init_states = next_states.numpy() # if not done, continue batch collection from last states

    batch_s = torch.stack(batch_s).numpy()
    batch_a = torch.stack(batch_a).numpy()
...
def learn(batch_s, batch_a, batch_v_t, model, optimizer):
    ...
    batch_v_t_ = torch.from_numpy(batch_v_t.copy()).float() # pytorch's problem of negative stride -> require .copy() to create new numpy
    ...
    return total_loss.detach().numpy(), mus.detach().numpy(), stds.numpy()
